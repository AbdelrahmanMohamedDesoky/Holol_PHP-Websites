...
<html lang="en">
    ...
    <body>
        <div class="course">
            <div class="container">
                <form method="post" action="editOnlineCourse.php" enctype="multipart/form-data">
                    <h2>Edit/Delete Courses</h2>
                    <label>Please Select a Course :</label> <br>
                    <select name = "OnId">
                        <?php
                        // echos all online courses titles inside a select tag and gets the input via OnId as value
                        $catagoryQuery = "SELECT * FROM onlinecou;";
                        $result = mysqli_query($db, $catagoryQuery);

                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_array($result)) {
                                $OnId = $row['OnId'];
                                $OnName = $row['courseName'];
                                echo "<option value=\"$OnId\">$OnName</option>";
                            }
                        }
                        ?>
                    </select>
                    ...
                    if (isset($_POST['showData'])) {
                        if (isset($_POST['OnId'])) {
                            $offId = cleanSQL($_POST['OnId']);
                            $courseQuery = "SELECT * FROM onlinecou WHERE OnId='$OnId';";
                            ...
                            if ($result) {
                                ...
                                $description = $row['OnDis'];
                                $courseLink = $row['OnLink'];
							 ...
							 <textarea name='description'>$description</textarea><br><br>
                                                         ...
                                                         <textarea name='link' required>$courseLink</textarea><br><br>
                        ...
                        if (checkForErrors($_POST['name'], $_POST['price'], $_POST['hours'], $_POST['description'], $_FILES['img'],$_POST['link'])) {
                            ...
                            $offlineCourseQuery = "UPDATE `onlinecou` SET `courseName` = '$name', `coursePrice` = '$price', `courseNumOfHours` = '$hours', `OnDis`= '$description' , `OnImage` = '$imgName'  WHERE `onlinecou`.`OnId` = $OnId;";
                        ...
                        $OnId = cleanSQL($_POST['OnId']);
                        $deleteQuery = "DELETE FROM `onlinecou` WHERE `onlinecou`.`OnId` = $OnId";
