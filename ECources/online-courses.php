                ...
                <div class="container">
                    ...
                    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-2">
                        <ul class="nav navbar-nav navbar-right">
                            ...
                            <?php
                            // echos profile button if user is logged in
                            if (!isset($_SESSION)) {
                                session_start();
                            }
                            if (isset($_SESSION['username'])) {
            ...
            <div class="container ">
                <div class="course-category row">
                    <div class="cat-drop col-xs-4">
                        <ul class="list-group">
                            <?php
                            // echos the categories in the left as unordered list items
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_array($result)) {
                                    $catId = $row['CatId'];
                                    $catName = $row['CatName'];
                                    echo "<li class='list-group-item'><a href='online-courses.php?catName=$catName'>$catName</a></li>";
                                }
                            }
                            ?> 
                        </ul>
                ...
                if (!isset($_GET['catName'])) {
                    ...
                    while ($category = mysqli_fetch_array($result)) {
                        ...
                        $coursesQuery = "SELECT * FROM onlinecou WHERE CatId='$catId';";
                        ...
                        while ($courseRow = mysqli_fetch_array($coursesResult)) {
                            ...
                            $courseDescription = $courseRow['OnDis'];
                            $coursePrice = $courseRow['coursePrice'];
                            $courseHours = $courseRow['courseNumOfHours'];
                            $courseId = $courseRow['OnId'];
                            $imgName = $courseRow['OnImage'];
					...
					<div class='info row'>
                            <form action = 'regOnlineCourse.php' method='GET' class='col-xs-12'> 
                                    <button class='btn hvr-sweep-to-right'>Enroll in this Course</button> 
                                    <hr><a href='viewCourse.php?courseId=$courseId'> View Course</a></li>
                                    <hr>    
