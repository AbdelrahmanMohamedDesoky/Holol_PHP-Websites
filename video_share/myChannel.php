				...
				<div class='col-sm-12 show-grid-left main-grids'>
					<div class='recommended'>
						<div class='recommended-grids english-grid'>
							...
							if($guest->getChannelId($_SESSION['username']) != -1){
									$channelId = $guest->getChannelId($_SESSION['username']);
									...
									$channelImage = $channelRow['channelImg'] . ".jpg?" . uniqid();
									echo "
										<div class='recommended-grids english-grid'>
										<!-- Channel Header Here !-->
										<div style='width:100%;height:300px;margin-bottom:20px;background-image:url(./images/$channelImage);background-size:cover;'>
								  	...
								  	if(!empty($_SESSION['userId'])){
										if($_SESSION['userId'] != $channelUserId){
											if($user->getSubscribtionState($channelId)){
												echo "<button class='btn btn-info' style='color:#fff;float:right;border-radius:10%;border:1px solid #000;margin-right:30px;margin-top:25px;opacity:1;font-size:24px;padding:10px 20px;'>Unsubscribe</button>";
											} else {
												echo "<button class='btn btn-info' style='color:#fff;float:right;border-radius:10%;border:1px solid #000;margin-right:30px;margin-top:25px;opacity:1;font-size:24px;padding:10px 20px;'>Subscribe</button>";
											}
										}
								  	}
								  	echo "</div></div>";
